...
public class AuthController {
	...
	public ResponseEntity<?> startGoogleLogin() {
		if (googleClientId == null || googleClientId.isBlank()) {
			return ResponseEntity.status(500).body(Map.of("error", "Google OAuth non configure: client-id manquant"));
		}
	...
	public ResponseEntity<?> googleCallback(String code, String error) {
		...
		try {
			...
			return ResponseEntity.status(302)
					.location(URI.create(frontendUrl + "?google_error=" + urlEncode("oauth_failed")))
					.build();
		}
	}
	...
	public ResponseEntity<?> loginWithGoogle(@RequestBody GoogleAuthRequest request) {
		if (request.googleSubject() == null || request.googleSubject().isBlank() || request.email() == null || request.email().isBlank()) {
			return ResponseEntity.badRequest().body(Map.of("error", "googleSubject et email sont obligatoires"));
		}

		User user = userRepository.findByGoogleSubject(request.googleSubject())
				.orElseGet(() -> userRepository.findByEmail(request.email()).orElse(null));

		if (user == null) {
			user = new User();
			user.setFullName(request.fullName() == null || request.fullName().isBlank() ? "Utilisateur Google" : request.fullName());
			user.setEmail(request.email());
	...
	private Map<String, Object> exchangeCodeForToken(String code) throws IOException, InterruptedException {
		...
		if (response.statusCode() < 200 || response.statusCode() >= 300) {
			throw new IllegalStateException("Echec token Google: " + response.statusCode());
		}
	...
	public record GoogleAuthRequest(String googleSubject, String email, String fullName) {
	}
}
